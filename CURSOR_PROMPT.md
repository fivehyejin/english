...
- `curriculum-m2.json` (학습 데이터, Day 1~5)
- `prototype/english_review.html` (참고용 — 비주얼은 따르지 말 것)
...
`CHANGELOG.md`를 먼저 읽어줘. 이 프로젝트는 "영어 복습 관리 앱"에서 **"영어 학습 정리 노트"**로 전면 재설계됐어. 이전 버전(v1)에 있던 복습 체크, 망각곡선, 북마크, 플래시카드, Exam Mode 같은 건 **전부 제거**됐어. `prototype/english_review.html`은 v1 UI라 비주얼·기능 모두 참고 안 하는 게 좋아. 데이터 구조와 음성 재생 로직 정도만 참고해.
...
hooks/
├── useLastViewedDay.ts
└── usePracticeState.ts           # difficultExamples 읽기/쓰기
...
`curriculum-m2.json`을 `data/`에 두고 TypeScript로 import. `types.ts`는 `DATA_SCHEMA.md` 내용 그대로 복사.
...
2. `prototype/english_review.html`의 UI를 그대로 옮기지 말 것 — 이건 v1이고 구조가 다름
...
- [ ] `/practice/result` 정답률 + 틀린 문제 리스트 표시, 누적 저장 없음
- [ ] 모바일 375px에서 레이아웃 OK
- [ ] 라이트/다크 양쪽 가독성 OK
- [ ] `pnpm build` 에러 없음
...
2. `types.ts` + `data/curriculum-m2.json` 연결
...
1. **누적 통계 저장 금지** — 정답률·세션 수·streak·날짜별 기록 등 일절 저장 X. 세션 끝나면 결과는 사라짐 (다음 세션엔 영향 X)
2. **홈 페이지 수정 최소화** — Day 연습은 Day 내부 기능. 홈엔 흔적 X. 전체 연습도 홈엔 secondary 톤 버튼 하나만 추가. 카운트·뱃지·알림 금지.
3. **성취 게이미피케이션 금지** — 점수, 레벨, 뱃지, 애니메이션 축하, 종이 꽃가루 효과 등 금지. 결과는 담담하게 숫자만.
4. **어려움 표시를 강조하지 말 것** — 노트의 점은 조용하게. 빨강 X, 배지 X, 숫자 카운터 X.
5. **외웠음/숙달 표시 금지** — "이 예문은 마스터" 같은 건 없음. 어려움 플래그의 **없음/있음** 두 상태만 존재.
...
2. `data/`에 추가 → `lib/curriculum.ts`의 `CURRICULA` 배열에 추가
...
3. `prototype/english_review.html`은 **굳이 걸지 말 것** (v1이라 혼란 야기 가능). 필요하면 나중에.
4. shadcn init 질문 답변:
   - Style: **New York**
   - Base color: **Slate**
   - CSS variables: **Yes**
